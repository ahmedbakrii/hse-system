...
export default function SupervisorDashboard() {
  ...
  useEffect(() => {
    // التأكد إن اللي فاتح الصفحة دي مشرف مسجل دخول
  ...
  async function fetchData(name) {
    // جلب كل المشغلين عشان البحث السريع
    const { data: ops } = await supabase.from("operators").select("*");
    if (ops) setOperators(ops);

    // جلب المخالفات اللي المشرف ده بس هو اللي سجلها
    const { data: hist } = await supabase
      .from("operator_history")
      .select("*")
      .ilike("details", `%${name}%`) // الفلترة باسم المشرف
  ...
  // فلترة نتائج البحث (بنعرض أول 5 نتائج بس عشان شاشة الموبايل)
          ...
          {searchQuery && (
            <div className="mt-3 space-y-2">
              {filteredOps.length > 0 ? (
                ...
                <div className="text-center p-3 text-sm font-bold text-red-500 bg-red-50 rounded-lg">لم يتم العثور على مشغل بهذا البحث ❌</div>
              )}
            </div>
          )}
        </div>

        {/* قسم سجل مخالفات المشرف */}
                ...
                <div key={record.id} className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  ...
                  <div className="flex items-start gap-2 mt-2">
                    ...
                    <p className="text-sm font-bold text-slate-700 leading-tight">
                      {/* مسح اسم المشرف من العرض لأنه عارف إنه هو اللي سجلها */}
